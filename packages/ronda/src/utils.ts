import { upper, pascalCase } from "change-case";
...
export function getModuleBasePaths(
  moduleName: string,
  moduleSourcePath: string
) {
  return getModuleBaseFiles(moduleName).map(
    (fileName) => `${moduleSourcePath}/${fileName}`
  );
}
...
export function renameHeaderNamespace(
  srcModuleName: string,
  dstModuleName: string,
  dstModuleSourcePath: string
) {
...
export async function refactorModuleBaseFiles(
  srcModuleName: string,
  dstModuleName: string,
  dstModuleSourcePath: string
) {
  const srcModuleBasePaths = getModuleBasePaths(
    srcModuleName,
    dstModuleSourcePath
  );
  const dstModuleBasePaths = getModuleBasePaths(
    dstModuleName,
    dstModuleSourcePath
  );

  await Promise.all(
    srcModuleBasePaths.map((path, i) => move(path, dstModuleBasePaths[i]))
  );
  // Replace module name
  await replace({
...
export async function pickExistingModule(
  pluginFileJSON: any,
  sourcePath: string
) {
...
export async function createModule(
  ...
  await refactorModuleBaseFiles(
    srcModuleName,
    dstModuleName,
    dstModuleSourcePath
  );
  await renameHeaderNamespace(
    srcModuleName,
    dstModuleName,
    dstModuleSourcePath
  );
